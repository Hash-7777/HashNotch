...
your afternoon: what you are listening to, which app has your microphone or camera open
...
| Temperatures | On-die sensors through the IOKit HID event system where a Mac has them, and the System Management Controller — `AppleSMC` — where it does not | The temperature readout. Read-only, both ways. The HID interface exists only on Apple Silicon and gives sensors with names; the SMC is on every Mac and names its sensors with four-character keys, so it is asked second and only when the first finds nothing. For temperatures, neither is asked for anything else: the SMC carries fans, voltages and currents too, and only keys beginning with T are read for this row. (The power row below reads one more SMC key, and says which.) Nothing is asked of either while the panel is shut. |
| Power draw | One key from the System Management Controller, `PSTR` — the machine's total draw in watts — and, where a Mac does not have it, the battery controller's own telemetry (`AppleSmartBattery` in the I/O Registry, the figure `ioreg` shows) or the battery's voltage and current while it is running down. The connected charger's rating comes from the same call the battery row uses | How much power the whole Mac is using right now, and a graph of the last half-minute. It is what the Mac as a whole draws — not what the charger delivers or what the battery gives up — so it means the same thing on either. It turns red at 90% of the connected charger's rating, where the battery starts making up the difference while plugged in, and never on battery. Read-only, no permission, once a second and only while the panel is open. A Mac that answers none of these gets no power row rather than a guessed figure. |
...
| **Microphone and camera in use** | CoreAudio is asked one question per audio process: *is this process running an input stream?* The answer is a **boolean**. No audio is opened, no samples are read, and **the app holds no microphone permission** — reading this flag is not using the microphone, the way seeing a door is shut is not going through it | The live dot and the call timer. It shows that an app has your microphone open, which app it is, and for how long. It cannot know who you are speaking to, whether anyone is speaking, or what is said — **nothing is listened to, recorded or transcribed, ever**, and there is nothing here that could be extended to do so: the API returns a flag and a process id. The process id becomes a name and an icon through the list of running applications. Only real apps count, because macOS's own dictation service holds an input stream open permanently — and that filter is for *being an app*, not a list of meeting apps by name, so FaceTime, Zoom, Teams, Meet in a browser, a voice memo or a game all work without any of them being named anywhere. Dictation itself is not detectable this way and is deliberately not guessed at. **The camera is read the same way, and answers less.** CoreMediaIO is asked one question per camera device: *is this camera running somewhere?* Again a boolean. No video is opened, no frame is read, nothing is recorded or looked at, and **the app holds no camera permission** — measured: this reads with no permission and no prompt. What CoreMediaIO does NOT publish is a list of processes, so unlike the microphone a camera **cannot be attributed to an app at all**. A camera on its own is therefore shown as live and named as nothing. Guessing that it belongs to whichever app last held the microphone would usually be right, and is deliberately not done: naming the wrong app on a row about being watched is worse than naming none. One more thing it cannot tell you — whether you are muted. It reports that an app **holds** the device; many apps, Google Meet among them (measured), keep the microphone open and mute in software, so the readout stays on. Below macOS 14.4 the per-process list does not exist; the app then falls back to a single device-wide question — is *anything* using the default input — so the dot still appears but names nothing, which is the honest answer when nothing can be attributed. |
...
Everything else, including the microphone and camera readout, uses documented public

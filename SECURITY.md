...
| Temperatures | On-die sensors through the IOKit HID event system where a Mac has them, and the System Management Controller — `AppleSMC` — where it does not | The temperature readout. Read-only, both ways. The HID interface exists only on Apple Silicon and gives sensors with names; the SMC is on every Mac and names its sensors with four-character keys, so it is asked second and only when the first finds nothing. Neither is asked for anything but temperature: the SMC carries fans, voltages and currents too, and only keys beginning with T are read. Nothing is asked of either while the panel is shut. |
